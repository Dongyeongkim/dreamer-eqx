...
class VecDmEnvWrapper(dm_env.Environment):
    ...
    def render(self):
        ...
        return jax.numpy.array(
            image.render_array(sys, [VState(q[i], qd[i]) for i in range(self.num_env)], self.width, self.height))
...
if __name__ == '__main__':
    ...
    for i in range(100):
        print(f'step {i}')
        actions = jax.random.uniform(jax.random.key(i), (num_env, *env.action_spec().shape))
